...
export async function seedShiftApplications() {
  ...
  // Get all requirements to properly match staff roles with available positions
  const requirements = await shiftRequirementRepository.find();
  
  const applications: any[] = [];
  
  // Staff roster with their qualified roles:
  // Servers: Mike Davis (3), Lisa Anderson (4), James Wilson (5), Sarah Johnson (2)
  // Cooks: Carlos Rodriguez (6), Maria Garcia (7) 
  // Bartender: James Wilson (5) - can also serve
  // Manager: Sarah Johnson (2) - can also serve
  
  // Create realistic applications where staff apply for roles they're qualified for
  // Focus on shifts 31+ (days 10-14) where fewer assignments exist
  
  // Find server requirements for shifts 31-42 (days 10-14)
  const serverRequirements = requirements.filter(req => 
    req.roleName === 'server' && req.shiftId >= 31 && req.shiftId <= 42
  );
  
  // Find cook requirements for shifts 31-42
  const cookRequirements = requirements.filter(req => 
    req.roleName === 'cook' && req.shiftId >= 31 && req.shiftId <= 42
  );
  
  // Find cleaner requirements for shifts 31-42  
  const cleanerRequirements = requirements.filter(req => 
    req.roleName === 'cleaner' && req.shiftId >= 31 && req.shiftId <= 42
  );
  
  // Create server applications - servers can apply for server roles
  if (serverRequirements.length > 0) {
    // Mike Davis (server) applies for first available server shift
    applications.push({
      shiftId: serverRequirements[0].shiftId,
      staffId: 3, // Mike Davis
      status: ShiftApplicationStatus.APPLIED,
      appliedAt: new Date(Date.now() - 6 * 60 * 60 * 1000),
      desiredRequirementId: serverRequirements[0].id
    });
    
    // Lisa Anderson (server) applies for second server shift if available
    if (serverRequirements.length > 1) {
      applications.push({
        shiftId: serverRequirements[1].shiftId,
        staffId: 4, // Lisa Anderson
        status: ShiftApplicationStatus.APPLIED,
        appliedAt: new Date(Date.now() - 4 * 60 * 60 * 1000),
        desiredRequirementId: serverRequirements[1].id
      });
    }
    
    // James Wilson (server/bartender) applies for third server shift if available
    if (serverRequirements.length > 2) {
      applications.push({
        shiftId: serverRequirements[2].shiftId,
        staffId: 5, // James Wilson
        status: ShiftApplicationStatus.APPROVED, // Some approved for variety
        appliedAt: new Date(Date.now() - 12 * 60 * 60 * 1000),
        desiredRequirementId: serverRequirements[2].id
      });
    }
    
    // Sarah Johnson (manager/server) applies for fourth server shift if available
    if (serverRequirements.length > 3) {
      applications.push({
        shiftId: serverRequirements[3].shiftId,
        staffId: 2, // Sarah Johnson  
        status: ShiftApplicationStatus.APPLIED,
        appliedAt: new Date(Date.now() - 2 * 60 * 60 * 1000),
        desiredRequirementId: serverRequirements[3].id
      });
    }
  }
  
  // Create cook applications - cooks apply for cook roles
  if (cookRequirements.length > 0) {
    // Carlos Rodriguez (cook) applies for first cook shift
    applications.push({
      shiftId: cookRequirements[0].shiftId,
      staffId: 6, // Carlos Rodriguez
      status: ShiftApplicationStatus.APPLIED,
      appliedAt: new Date(Date.now() - 3 * 60 * 60 * 1000),
      desiredRequirementId: cookRequirements[0].id
    });
    
    // Maria Garcia (cook) applies for second cook shift if available
    if (cookRequirements.length > 1) {
      applications.push({
        shiftId: cookRequirements[1].shiftId,
        staffId: 7, // Maria Garcia
        status: ShiftApplicationStatus.APPLIED,
        appliedAt: new Date(Date.now() - 1 * 60 * 60 * 1000),
        desiredRequirementId: cookRequirements[1].id
      });
    }
  }
  
  // Add one rejection example for testing
  if (serverRequirements.length > 4) {
    applications.push({
      shiftId: serverRequirements[4].shiftId,
      staffId: 3, // Mike Davis rejected
      status: ShiftApplicationStatus.REJECTED,
      appliedAt: new Date(Date.now() - 8 * 60 * 60 * 1000),
      desiredRequirementId: serverRequirements[4].id
    });
  }
  
  await shiftApplicationRepository.save(applications);
  console.log(`Created ${applications.length} shift applications with proper role matching`);
}
