...
import { TimeOffRequest, User } from '../../shared/models';
import { AppDataSource } from '../../shared/data-source';
import { TimeOffStatus, StaffStatus } from '../../shared/models/enums';
...
export const getTimeOffRequests = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const staffIds = [...new Set(requests.map((r: { staffId: any; }) => r.staffId))];
    const staffMembers = await userRepository.findByIds(staffIds);
    interface StaffMap extends Map<number, User> {}
    const staffMap: StaffMap = new Map<number, User>(staffMembers.map((staff: User): [number, User] => [staff.id, staff]));

    interface FormattedTimeOffRequest {
      id: number;
      staffId: number;
      staffName: string;
      startDate: string;
    ...
    const formattedRequests: FormattedTimeOffRequest[] = requests.map((request: TimeOffRequest): FormattedTimeOffRequest => {
      const staff: User | undefined = staffMap.get(request.staffId);
      return {
      id: request.id,
