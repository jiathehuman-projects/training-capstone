// Standalone test file for order validation (no database required)
import { validateOrderItem } from '../../controllers/order';

// Mock MenuItem interface for testing
...
describe('Order Item Validation (Standalone)', () => {
  let mockMenuItem: MockMenuItem;
  ...
  describe('validateOrderItem', () => {
    it('should validate a valid order item', () => {
      ...
      const result = validateOrderItem(orderItem, mockMenuItem as any);
    ...
    it('should handle edge case quantities', () => {
      ...
      const validResult = validateOrderItem(validItem, mockMenuItem as any);
      expect(validResult.isValid).toBe(true);

      // Test one over stock
      const invalidItem = { menuItemId: 1, quantity: 2 };
      const invalidResult = validateOrderItem(invalidItem, mockMenuItem as any);
